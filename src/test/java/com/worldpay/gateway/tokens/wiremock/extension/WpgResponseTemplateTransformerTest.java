...
import static com.worldpay.gateway.tokens.wiremock.testsupport.NoFileSource.noFileSource;
...
/**
 * Tests for {@link WpgResponseTemplateTransformer}.
 * Heavily based on a test from the WireMock code base.
 */
public class WpgResponseTemplateTransformerTest {

    private WpgResponseTemplateTransformer transformer;

    @Before
    public void setup() {
        transformer = new WpgResponseTemplateTransformer();
    }

    @Test
    public void applyGlobally_isTrue() {
        assertTrue(transformer.applyGlobally());
    }

    @Test
    public void getName_returnsCorrectName() {
        assertEquals("wpg-response-template", transformer.getName());
    }
    ...
    public void queryParameters() {
        ResponseDefinition transformedResponseDef = transform(mockRequest()
                ...
                aResponse().withBody(
                        "Multi 1: {{request.query.multi_param.[0]}}, "
                                + "Multi 2: {{request.query.multi_param.[1]}}, "
                                + "Single 1: {{request.query.single-param}}"
                )
        );
    ...
    public void requestHeaders() {
        ResponseDefinition transformedResponseDef = transform(mockRequest()
                        .url("/things")
                        .withHeader("X-Request-Id", "req-id-1234")
                        .withHeader("123$%$^&__why_o_why", "foundit"),
                aResponse().withBody(
                        "Request ID: {{request.headers.X-Request-Id}}, "
                                + "Awkward named header: {{request.headers.[123$%$^&__why_o_why]}}"
                )
        );
    ...
    public void singleValueTemplatedResponseHeaders() {
        ResponseDefinition transformedResponseDef = transform(mockRequest()
                        .url("/things")
                        .withHeader("X-Correlation-Id", "12345"),
    ...
    public void multiValueTemplatedResponseHeaders() {
        ResponseDefinition transformedResponseDef = transform(mockRequest()
                        .url("/things")
                        .withHeader("X-Correlation-Id-1", "12345")
                        .withHeader("X-Correlation-Id-2", "56789"),
    ...
    public void customHelper() {
        ...
        transformer = new WpgResponseTemplateTransformer("string-length", helper);
    ...
    public void proxyBaseUrl() {
        ResponseDefinition transformedResponseDef = transform(mockRequest()
                        .url("/things")
                        .withHeader("X-WM-Uri", "http://localhost:8000"),
